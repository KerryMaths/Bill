(function() {
  ...
  describe('StatementFactory', function(){

    var getStatementFactory,
      $httpBackend = null,
      billResponse = {
          "statement": {
    ...
    beforeEach(function(){
      ...
      inject(function(_getStatementFactory_,_$httpBackend_){
        getStatementFactory = _getStatementFactory_;
    ...
    describe('initialisation', function(){ 
      it('should perform Http request', function(){

        var data = $httpBackend.whenGET('https://still-scrubland-9880.herokuapp.com/bill.json').respond(billResponse);

        $httpBackend.flush();
        expect(getStatementFactory).toBeTruthy();
      });
    });

    describe('Returning data', function(){ 
      it('should return JSON data from http request', function(){

        var data = $httpBackend.whenGET('https://still-scrubland-9880.herokuapp.com/bill.json').respond(billResponse);

        $httpBackend.flush();
        expect(getStatementFactory.$$state.value).toEqual(billResponse);
        expect(getStatementFactory.$$state.value.total).toEqual(136.03);
      });
    });
